...
- 📱 **多渠道推送**：支持企业微信、邮件、sms_web、POST JSON、Bark、GET、钉钉、PushPlus、Server酱、自定义模板、飞书、Gotify 与 Telegram
- 🧩 **低内存长短信合并**：最多同时缓存 2 组、每组 8 段，30 秒超时后标注缺段并转发
- 🛡️ **号码黑名单与管理员命令**：支持号码过滤，以及管理员短信远程发短信/重启
- 📶 **模组管理**：Web 查询信号、运营商、IMEI，切换飞行模式、软/硬重启及 Ping 保号
- 📜 **网页日志与 AT 控制台**：固定大小环形日志，避免日志持续消耗 RAM
...
| 队列 | `/queue` | 查看待重试的短信队列 |
| 模组 | `/modem` | AT 控制台、信号/运营商/IMEI、飞行模式、重启与 Ping |
| 日志 | `/logs` | 查看固定容量的运行日志 |
...
- HTTP 服务器 URL
- 各推送渠道的启用/禁用开关
- 最多 3 个扩展推送通道及其 URL、Token、Secret 和模板
- 管理员号码与号码黑名单
- Web 管理账号密码
...
| 2 | Bark | URL |
| 3 | GET | URL |
| 4 | 钉钉 | URL，Key 1 为 Secret（可空） |
| 5 | PushPlus | Key 1 为 Token，Key 2 为 channel（可空） |
| 6 | Server酱 | Key 1 为 SendKey |
| 7 | 自定义模板 | URL，模板支持 `{sender}`、`{message}`、`{timestamp}` |
| 8 | 飞书 | URL，Key 1 为 Secret（可空） |
| 9 | Gotify | URL，Key 1 为 Token |
| 10 | Telegram | Key 1 为 Chat ID，Key 2 为 Bot Token；URL 可空 |

每个通道使用一个重试状态位，失败后只重试失败通道，不会重复推送已成功的渠道。

## 管理员短信与黑名单

配置管理员号码后，只接受该号码发送的以下精确命令：

- `SMS:目标号码:短信内容`：远程发送短信。
- `RESET`：硬重启蜂窝模组后重启 ESP32。

号码比较会忽略 `+86` 前缀。黑名单支持每行一个号码或逗号分隔；命中的短信不会进入任何推送或重试队列。
...
| 扩展推送通道 | 3 个（可用 `MAX_PUSH_CHANNELS` 覆盖） |
| 长短信缓存 | 2 组 × 8 段，每段最多 191 字节 |
| 网页日志 | 3072 字节环形缓冲 |
| 最大重试次数 | 5 次 |
| 重试间隔 | 60 秒 |
...
在 ESP32-C3、4MB Flash 的 PlatformIO 实测构建中，静态 RAM 使用约 64KB（19.6%），Flash 使用约 1.02MB（应用分区的 77.7%）。若设备资源更紧张，可在 `platformio.ini` 的 `build_flags` 中降低 `SMS_QUEUE_SIZE`、`MAX_PUSH_CHANNELS`、`MAX_CONCAT_MESSAGES` 或 `WEB_LOG_SIZE`。

## PlatformIO 构建

仓库已包含 `platformio.ini`：

```bash
python -m platformio run
```

首次构建前复制 `config-example.h` 为 `config.h`。默认环境为 `esp32-c3-devkitm-1`；使用其他 ESP32 板卡时请修改 `board`，并确认 UART 与 `MODEM_EN_PIN` 引脚。
